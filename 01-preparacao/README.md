...
> [!IMPORTANT]
ALTERAÇÃO (Nota sobre Nomenclatura): Optamos pelo `Internal naming scheme` em vez de `CHARMM` para evitar a inserção de indicadores `TER` no arquivo `.prq`. Isso previne a interrupção do processo pela caixa de erro *"Unparametrized Components"* no AutoPSF (VMD) durante a montagem da caixa (**Etapa 4**).
