...
> [!IMPORTANT]
ALTERAÇÃO (Nota sobre Nomenclatura): Optamos pelo `Internal naming scheme` em vez de `CHARMM` para evitar a inserção de indicadores `TER` no arquivo. Isto previne erros de *"Unparametrized Components"* no AutoPSF (VMD) durante a montagem da caixa na **Etapa 4**.
